...
Class ddwClass{
    ...
    public static function customerDDW() {
        return [" "] + Customer::where('Deleted', 0)->where('IsCompany', 1)->whereNull('WebUserName')->orderBy('Name')->pluck('Name', 'Id')->toArray();
    }

    public static function customerContactDDW($customer) {
    ...
    public static function customerAddressDDW($customer)
    {
    ...
    public static function customerPaymentMethodDDW()
    {
        return PaymentMethod::where('UseAlways', 1)
            ->orWhere('Id', shoppingCartClass::customerPaymentMethod())
