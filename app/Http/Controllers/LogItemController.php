...
//                }
    ...
    public function indexData($startDate, $endDate, $customer, $user)
    {
    ...
    public function index(Request $request)
    {
        if( myUser::check() ){
            ...
            if ($request->ajax()) {
                ...
                $startDate = date('Y-m-d H:m:s', strtotime('now - 1 day'));
                $endDate   = date('Y-m-d H:m:s', strtotime('now + 1 day'));
                $customer  = 0;
                $user      = 0;
     ...
     * @return Response
     */
    public function indexBetween(Request $request, $startDate, $endDate, $customer, $user)
    {
        if( myUser::check() ){

            if ($request->ajax()) {

                return $this->dwData($this->indexData(date('Y-m-d H:m:s', strtotime($startDate)), date('Y-m-d H:m:s', strtotime($endDate)), $customer, $user));
    ...
    public function show($id)
    {
        ...
        return view('log_items.show')->with('logItem', $logItem);
    }
