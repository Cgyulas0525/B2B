    ...
    <div class="row">
        @if ( $coffers->count() > 0 )
            @foreach ( $coffers as $coffer)
                <div class="{{ App\Classes\utilityClass::offerColLg($coffers->count()) }}">
                    <!-- small box -->
    ...
    <script type="text/javascript">
        $(function () {
            ...
            var table = $('.customer-table').DataTable({
                ...
                filter: false,
                columns: [
                    {title: 'Megrendelés szám', data: 'VoucherNumber', width:'30%', name: 'VoucherNumber'},
                    {title: 'Dátum', data: 'VoucherDate', render: function (data, type, row) { return data ? moment(data).format('YYYY.MM.DD') : ''; }, sClass: "text-center", width:'10%', name: 'VoucherDate'},
                    {title: 'Netto', data: 'NetValue', render: $.fn.dataTable.render.number( '.', ',', 0), sClass: "text-right", width:'15%', name: 'NetValue'},
                    {title: 'ÁFA', data: 'VatValue', render: $.fn.dataTable.render.number( '.', ',', 0), sClass: "text-right", width:'15%', name: 'VatValue'},
                    {title: 'Bruttó', data: 'GrossValue', render: $.fn.dataTable.render.number( '.', ',', 0), sClass: "text-right", width:'15%', name: 'GrossValue'},
                    {title: 'Pénznem', data: 'currencyName', sClass: "text-center", width:'5%', name: 'currencyName'},
                    {title: 'Tétel', data: 'tetelszam', render: $.fn.dataTable.render.number( '.', ',', 0), sClass: "text-right", width:'10%', name: 'tetelszam'},
                ],
